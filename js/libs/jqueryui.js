...
function (a, b) {
  a.widget("ui.draggable", a.ui.mouse, {
    ...
    destroy: function () {
      if ( !! this.element.data("draggable")) {
        this.element.removeData("draggable")
          .unbind(".draggable")
          .removeClass("ui-draggable ui-draggable-dragging ui-draggable-disabled"), this._mouseDestroy();
        return this
      }
    },
    _mouseCapture: function (b) {
      var c = this.options;
      if (this.helper || c.disabled || a(b.target)
        .is(".ui-resizable-handle")) return !1;
      this.handle = this._getHandle(b);
      if (!this.handle) return !1;
      c.iframeFix && a(c.iframeFix === !0 ? "iframe" : c.iframeFix)
        .each(function () {
    ...
    _mouseUp: function (b) {
      this.options.iframeFix === !0 && a("div.ui-draggable-iframeFix")
        .each(function () {
    ...
    _createHelper: function (b) {
      ...
      d.parents("body")
        .length || d.appendTo(c.appendTo == "parent" ? this.element[0].parentNode : c.appendTo), d[0] != this.element[0] && !/(fixed|absolute)/.test(d.css("position")) && d.css("position", "absolute");
      return d
    },
  ...
  a.widget("ui.resizable", a.ui.mouse, {
    ...
    _mouseStart: function (b) {
      ...
      var i = a(".ui-resizable-" + this.axis)
        .css("cursor");
      a("body")
        .css("cursor", i == "auto" ? this.axis + "-resize" : i), f.addClass("ui-resizable-resizing"), this._propagate("start", b);
      return !0
    },
    ...
    _mouseStop: function (b) {
      ...
      a("body")
        .css("cursor", "auto"), this.element.removeClass("ui-resizable-resizing"), this._propagate("stop", b), this._helper && this.helper.remove();
      return !1
    },
        ...
        h = {
          ...
          a(b)
            ...
            var b = a(this),
              ...
              g = {}, i = e && e.length ? e : b.parents(c.originalElement[0])
  ...
  a.widget("ui.selectable", a.ui.mouse, {
    ...
    destroy: function () {
      this.selectees.removeClass("ui-selectee")
        ...
        .unbind(".selectable"), this._mouseDestroy();
      return this
    },
    _mouseStart: function (b) {
      var c = this;
      ...
      if (!this.options.disabled) {
        ...
        this.selectees = a(d.filter, this.element[0]), this._trigger("start", b), a(d.appendTo)
          ...
          if (d) {
            ...
            d.$element.removeClass(e ? "ui-unselecting" : "ui-selected")
...
function (a, b) {
  ...
  a.effects.animateClass = function (b, c, d, e) {
    ...
    createWrapper: function (b) {
      ...
      b.wrap(d), (b[0] === e || a.contains(b[0], e)) && a(e)
...
function (a, b) {
  a.effects.pulsate = function (b) {
    return this.queue(function () {
      ...
      times = (b.options.times || 5) * 2 - 1, duration = b.duration ? b.duration / 2 : a.fx.speeds._default / 2, isVisible = c.is(":visible"), animateTo = 0, isVisible || (c.css("opacity", 0)
        .show(), animateTo = 1), (d == "hide" && isVisible || d == "show" && !isVisible) && times--;
...
function (a, b) {
  a.widget("ui.accordion", {
    ...
    _completed: function (a) {
      ...
      this.running || (this.options.clearStyle && this.toShow.add(this.toHide)
        .css({
        height: "",
        overflow: ""
  ...
  a.widget("ui.autocomplete", {
    ...
    _resizeMenu: function () {
      ...
      a.outerWidth(Math.max(a.width("")
        .outerWidth() + 1, this.element.outerWidth(), this.element.closest('.c-widget-autocompletar')
        .outerWidth()))
    },
...
function ($, undefined) {
  ...
  function Datepicker() {
    ...
    this.regional["en"] = this.regional[""],
    this.regional["es"] = {
    closeText: 'Cerrar',
    prevText: '&#x3c;Ant',
    nextText: 'Sig&#x3e;',
...
$.widget("ui.selectmenu", {
  ...
  _create: function() {
    ...
    $(document).bind("mousedown.selectmenu-" + this.ids[0], function(event) {
      // Comprobamos que no llegue un combo sin scroll personalizado
      if ($(event.target).closest('.selectmenu-ul-wrapper').parent().hasClass('noTruncate')){
        return false;
      }
      if (self.isOpen && $(event.target).closest('.scrollbar').length === 0) {
        self.close( event );
      }
    });
  ...
  _init: function() {
    ...
    if (selectOptionData.length) {
       // se mueven los eventos del li al ul, para evitar crear muchos y que en explorer no se muera "tanto"
      this.list.on("mouseup.selectmenu", "li", function(event) {
            if (self._safemouseup && !self._disabled(event.currentTarget) && !self._disabled($( event.currentTarget ).parents( "ul>li." + self.widgetBaseClass + "-group " )) ) {
              var changed = $(this).data('index') != self._selectedIndex() || self.element[0].selectedIndex === -1; //var changed = $(this).data('index') != self._selectedIndex();
          ...
          .bind("click.selectmenu", "li", function() {
            return false;
          })
      ...
      for (var i = 0; i < selectOptionData.length; i++) {
        ...
        var thisLi = $('<li/>', thisLiAttr)
          ...
          /* estos eventos se han movilo al ul, cuando pase un tiempo y no falle se puede eliminar esto
...
*/
    ...
    var maxH = o.maxHeight ? Math.min( o.maxHeight, winH ) : winH / o.listResizer;
  ...
  _typeAhead: function( code, eventType ) {
    ...
    if ( nextIndex !== null ) {
      ...
      // up menu
      asociatedSelectId =this._optionLis.parent().attr("id").split("-menu")[0];
      if($("#"+ asociatedSelectId).attr("disabled") !== "disabled"){
  ...
  open: function(event) {
    ...
    if ( self.newelement.attr("aria-disabled") != 'true' ) {
      ...
      //init scroll
      //self.list.closest('.scrollableParent').height(self.list.height());
      self.list.css('height', 'auto');

      // No inicializamos customScroll2 para los combos que lleven la clase noTruncate y ie <= 8 (p.e. altaCliente)
      if (self.element.parent().hasClass('noTruncate') && $.browser.msie && parseInt($.browser.version, 10) <= 8)  {
        self.setListWidthNoTruncate();
      } else {
        if ($B.appConfig.enableTabletFeatures && $B.capabilities.touch) {
          $B.ui.scrollElement(self.list); 
        } else{
          self.list.customScroll2({ scrollType: 'combo' });
          self.list.customScroll2('setPosition');
        } 
        //$B.ui.scrollElement(self.list);              
        self.setListWidth();
      }
      
      //set focus for navigation by keyboard
      if (self.list.find(':focus').length === 0) {
        if (self.list.find('li.ui-selectmenu-item-selected').length) {
          self.list.find('li.ui-selectmenu-item-selected a').focus();
        } else {
          self.list.find('li:eq(0) a').focus();
        }
      }
    }
  },
  ...
  _selectedIndex: function() {
    return Math.max(0, this.element[0].selectedIndex); //return this.element[0].selectedIndex;
  },
  ...
  _refreshPosition: function() {
    ...
    this.listWrap
      .removeAttr('style')
      .zIndex( /*this.element.zIndex() + 1*/ 150000 )
      .position({
  ...
  refreshScroll: function (event) {
    $(event.currentTarget).customScroll2('setPosition');
    /*var scrollableParent = $(event.currentTarget).closest('.scrollableParent');
    if (scrollableParent.data('tsb') !== undefined) {
      scrollableParent.data('tsb').thumbPosFromOffset();
    }*/
  },

  setListWidth: function () {
    var width = this.element.closest('.c-form-combo').width(),
      divWrapper = this.list.closest('.ui-selectmenu-menu');
    //reseteamos el ancho
    divWrapper.find('.scrollableParent, .overview, .viewport').add(divWrapper).width(width);
    this.list.width(width - 2);
  },

  setListWidthNoTruncate: function () {
    var width = this.element.closest('.c-form-combo').width(),
      divWrapper = this.list.closest('.ui-selectmenu-menu');
    //reseteamos el ancho para ie y en combos noTruncate    
    divWrapper.find('.scrollableParent, .overview, .viewport, .selectmenu-ul-wrapper').add(divWrapper).width(width);
    divWrapper.find('.ui-selectmenu-menu-dropdown').css({'width' : 'auto'});    
  }
